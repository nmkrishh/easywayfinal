...
const HeroWave = () => {
  ...
  useEffect(() => {
    ...
    const SCALE = 8; // Increased scale drastically reduces pixel loops fixing the lag
    ...
    const render = () => {
      ...
      if (SCALE > 1) {
        ctx.imageSmoothingEnabled = true; // Smooth upscaled pixels for gradient effect
  ...
  return <canvas ref={canvasRef} className="absolute inset-0 w-full h-full -z-10 object-cover" aria-hidden="true" />;
};
