...
export function Header({ onNavigate, pageKind = "default" }) {
	const [open, setOpen] = React.useState(false);
	const scrolled = useScroll(10);
	const isCompactPage = new Set(["dashboard", "ai-website-builder"]).has(pageKind);
	...
	const visibleLinks = isMobile ? [{ label: "Pricing", action: () => onNavigate("pricing") }] : links;
	...
	return (
		<header
			className={cn(
				'fixed top-0 inset-x-0 z-50 mx-auto w-full max-w-5xl border-b border-transparent md:rounded-md md:border md:transition-all md:ease-out',
				{
					'bg-background/95 supports-[backdrop-filter]:bg-background/50 border-border backdrop-blur-lg md:top-4 md:max-w-4xl md:shadow':
						scrolled && !open && !isCompactPage,
		...
		>
			...
			>
				<div onClick={() => onNavigate('home')} className="cursor-pointer flex items-center gap-2">
					<img src="/EasyWay-AI.png" alt="EasyWay AI Logo" className="h-7 brightness-0 invert" style={{ objectFit: 'contain' }} />
				</div>

				<div className="hidden items-center gap-2 md:flex">
					{visibleLinks.map((link, i) => (
						<button key={i} className={buttonVariants({ variant: 'ghost' })} onClick={link.action}>
							{link.label}
						</button>
					))}
					<Button variant="outline" onClick={() => onNavigate('dashboard')}>Dashboard</Button>
					<Button onClick={() => onNavigate('services')} className="rounded-full shadow-md">Start Building</Button>
				</div>
			...
			>
				...
				>
					...
					<div className="flex flex-col gap-2">
						<Button variant="outline" className="w-full" onClick={() => { onNavigate('dashboard'); setOpen(false); }}>
							Dashboard
						</Button>
						<Button className="w-full rounded-full shadow-md" onClick={() => { onNavigate('services'); setOpen(false); }}>Start Building</Button>
					</div>
				</div>
			</div>
		</header>
