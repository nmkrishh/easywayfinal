...
class AnimationController {
    ...
    public render() {
        ...
        // Transparent background so we can see the video underneath
        ctx.clearRect(0, 0, this.size, this.size)
        // Or if you want it black fade: ctx.fillStyle = 'rgba(0, 0, 0, 0.6)'; ctx.fillRect(0, 0, this.size, this.size);
...
class Star {
    ...
    render(p: number, controller: AnimationController) {
        ...
        if (q > 0) {
            ...
            // Allow casting without ts ignoring since we rely on controller internal properties via simple indexing
            const zCamera = (controller as any)['cameraZ'];
            const viewZm = (controller as any)['viewZoom'];

            const vx = (this.z - zCamera) * screenX / viewZm;
            const vy = (this.z - zCamera) * screenY / viewZm;
